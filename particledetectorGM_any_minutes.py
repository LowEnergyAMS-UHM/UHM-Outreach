#per aereo, ogni 12000 cicli(5minuti senza il show) salva una nuova
#foto con i dati del gps,non fa il real time ma aggiorna simplecv ogni 12000
#cicli. tolto lo sleep di 0.05.
...
if checkFix(button,ledrosso) is True:           #the program start after that the fix is set or after I decided to not wait for that
    ...
    print("\n")
    num_of_mins = raw_input("Enter a number n, so that this program takes a picture of the frame every n minutes")
    print("\n")
    num_of_cicles = int(num_of_mins) * 840
    ...
    #moving on the plane
      ...
      if cicles==num_of_cicles:#every n minutes
          first_thresh.show()
          stop = time.strftime('%H:%M:%S%p')          #Salvo il Tempo di fine esperimento
          date = time.strftime(' on %b %d %Y')
          image_path="%s-%s%s.jpg" % (start , stop , date)
          first_thresh.save(image_path)
          cont = str(0)
          gps_info = GPSdata(image_path, ledrosso)      #La funzione gpsdata restituisce l'array che salva con il nome di gps_info
